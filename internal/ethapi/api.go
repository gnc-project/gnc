...
import (
	...
	"github.com/gnc-project/gnc/consensus/clique"
...
//
// https://github.com/gnc-project/gnc/wiki/Management-APIs#personal_sign
...
//
// https://github.com/gnc-project/gnc/wiki/Management-APIs#personal_ecRecover
...
func (s *PublicBlockChainAPI) GetMinerInfo(ctx context.Context)(map[string]interface{},error){
	header,err:=s.b.GetCommitHeader(ctx)
	Difficulty,err:=s.b.GetCurrentBlock(ctx)
	if err!=nil{
		return nil,err
	}
	MinerInfoResult := map[string]interface{}{
		"baseTarget":        Difficulty,
		"blockHeight":       header.Number,
		"generationSignature": hex.EncodeToString(header.GenerationSignature),
	}
	return MinerInfoResult,nil
}

func (s *PublicBlockChainAPI) AddNonce(ctx context.Context,pid *big.Int,nonce *big.Int,number *big.Int)(map[string]interface{},error){
	gsinew, err := s.b.AddNonce(ctx,pid,nonce,number)
	if gsinew == nil || err != nil {
		return nil, err
	}
	gsinewResult := map[string]interface{}{
		"accountID":         gsinew.GetAccountID(),
		"deadline":          gsinew.GetDeadline(),
		"nonce":             gsinew.GetNonce(),
		"blockHeight":       gsinew.GetBlockHeight(),
		"generationSignature": hex.EncodeToString(gsinew. GetGenerationSignature()),
	}
	return gsinewResult, err 
}

func (s *PublicBlockChainAPI) GetAllGenerators(ctx context.Context)(map[string]interface{}){
	resule:=map[string]interface{}{}
	Generators:= s.b.GetAllGenerators(ctx)
	
	mg:=Generators.IterBuffered()
	
	for item := range mg{
			g:=item.Val.(*core.GeneratorStateImpl)
			val:=map[string]interface{}{
				"accountID":         g.GetAccountID(),
				"deadline":          g.GetDeadline(),
				"nonce":             g.GetNonce(),
				"blockHeight":       g.GetBlockHeight(),
				"generationSignature": hex.EncodeToString(g. GetGenerationSignature()),
			}
			resule[item.Key]=val
	  }
	return resule
...
//linzhaojie blockheader code
		...
		"generator":        (*hexutil.Big)(head.Generator),
		"baseTarget":        (*hexutil.Big)(head.Difficulty),
		"generationSignature":hexutil.Bytes(head.GenerationSignature),
		"rewardAmount":      (*hexutil.Big)(head.RewardAmount),
		"minerCapacity":     hexutil.Uint64(head.MinerCapacity),
		"requiredAmount":    (*hexutil.Big)(head.RequiredAmount),
