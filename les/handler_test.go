...
import (
	...
	"github.com/gnc-project/gnc/trie"
)
...
func testTransactionStatus(t *testing.T, protocol int) {
	...
	tx0, _ := types.SignTx(types.NewTransaction(types.Binary,0, userAddr1, big.NewInt(10000), params.TxGas, nil, nil), signer, bankKey)
	test(tx0, true, light.TxStatus{Status: core.TxStatusUnknown, Error: core.ErrUnderpriced.Error()})

	tx1, _ := types.SignTx(types.NewTransaction(types.Binary,0, userAddr1, big.NewInt(10000), params.TxGas, big.NewInt(100000000000), nil), signer, bankKey)
	...
	tx2, _ := types.SignTx(types.NewTransaction(types.Binary,1, userAddr1, big.NewInt(10000), params.TxGas, big.NewInt(100000000000), nil), signer, bankKey)
	tx3, _ := types.SignTx(types.NewTransaction(types.Binary,2, userAddr1, big.NewInt(10000), params.TxGas, big.NewInt(100000000000), nil), signer, bankKey)
