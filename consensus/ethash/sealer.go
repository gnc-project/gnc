...
var GeneratorImpl *core.GeneratorImpl
...
search:
	for {
		...
		default:
			...
			hei:=new(big.Int).Add(chain.CurrentBlock().Number(),big.NewInt(1))
			mg:=GeneratorImpl.Generators.IterBuffered()
			for item := range mg{
				generatorStateImpl:=item.Val.(*core.GeneratorStateImpl)
				if hei.Cmp(generatorStateImpl.GetBlockHeight()) == 0 {
					lastBlock:=chain.CurrentBlock()				
					elapsedTime:=GeneratorImpl.TimeService.GetEpochTime() - lastBlock.Time()
					// fmt.Println("EpochTime:",GeneratorImpl.TimeService.GetEpochTime())
					// fmt.Println("lastBlockTime:",lastBlock.Time())
					// fmt.Println("elapsedTime:",GeneratorImpl.TimeService.GetEpochTime() - lastBlock.Time())
					// fmt.Println("Deadline:",generatorStateImpl.GetDeadline())
					if big.NewInt(int64(elapsedTime)).Cmp(generatorStateImpl.GetDeadline())>0 {
	
					
					    header.Time=uint64(time.Now().Unix())
						
						header = types.CopyHeader(header)
						header.Generator=generatorStateImpl.GetAccountID()
						header.Nonce = types.EncodeNonce(generatorStateImpl.GetNonce().Uint64())
					...
					GeneratorImpl.Generators.Remove(item.Key)
					log.Warn("have new commitBlock ,remove old Generators:",item.Key)
				}
			}
		}
